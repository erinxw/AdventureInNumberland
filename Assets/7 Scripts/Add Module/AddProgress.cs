...
public class AddProgress : MonoBehaviour
{
        ...
        dbReference.Child("users").Child(userId).Child("progress").Child("addProgress").SetValueAsync(completedActivities)
            .ContinueWithOnMainThread(task =>
            {
                ...
                else if (task.IsCompleted)
                {
                    ...
                    Debug.Log("Progress saved to Firebase: " + completedActivities);
                }
            });
    }
    ...
    private void LoadProgressFromFirebase()
    {
        ...
        if (string.IsNullOrEmpty(userId))
        {
            ...
            completedActivities = PlayerPrefs.GetInt("addProgress", 0); // fallback
            UpdateProgressBar();
            ...
            .ContinueWithOnMainThread(task =>
            {
                ...
                else if (task.IsCompleted)
                {
                    ...
                    if (snapshot.Exists && int.TryParse(snapshot.Value.ToString(), out int savedProgress))
                    {
                        ...
                        Debug.Log("Loaded progress from Firebase: " + completedActivities);
                    }
                    else
                    {
                        completedActivities = 0;
                        Debug.Log("No saved progress found, starting at 0.");
                    }
                }
